...
wow-harness 有一个叫 `crystal-learn` 的能力。它的角色是**适应性免疫系统**。
...
**编号唯一性 chokepoint**——一个 git pre-commit hook，撞号直接拒绝提交。schema 级阻断，10 次历史撞号事件之后落地。这一条今天就同步到 wow-harness 里了，开箱可用。
...
wow-harness 从 Towow（通爻）的生产开发中提取出来。通爻是一个 Agent 协作协议项目。
